...
默认 `http://localhost:8080`。识图需要环境变量 `XAI_API_KEY`；没有 key 可手工填。

数据在浏览器 IndexedDB（`paiquote`），不经过服务器。
...
- 宋体 11、合并店面、红色合计
- 点「新建」必须马上开单；输入不能卡；要有明确保存（退出识别不能丢）

参考图在 `attachments/`（若仓库里没有，业务说明仍有效）。
